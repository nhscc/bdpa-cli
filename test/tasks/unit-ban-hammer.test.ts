...
import unleashBanHammer from 'universe:tasks/ban-hammer.ts';
...
const targets = ['drive', 'qoverflow'] as const satisfies ActualTargetProblem[];
...
function makeGetConfig(
  ...
  return function (key: string) {
    const actualKey = key.split('.').at(-1) as keyof typeof taskConfig;
